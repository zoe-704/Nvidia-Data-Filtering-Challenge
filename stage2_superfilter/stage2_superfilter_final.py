...
1. Calculating IFD scores using dual models for robust quality assessment
2. Creating a mixed dataset from top 70% and top 30% samples
3. Shuffling the final dataset for training diversity
4. Outputting clean data without internal scores for release

Features:
- Dual model IFD scoring with optimized tokenization
...
class SuperFilterFinal:
    """
    Final data curation system using dual model IFD scoring.
    ...
    def compute_ifd_scores_single_model(self, inputs, model) -> np.ndarray:
        """
        Compute IFD scores using a single model with pre-tokenized inputs.
    ...
    def compute_dual_ifd_scores(self, texts: List[str]) -> Tuple[np.ndarray, List[int]]:
        """
        Compute IFD scores using both models and average them.
        ...
        Returns:
            Tuple of (averaged IFD scores, token counts per text)
        """
        ...
        scores_1 = self.compute_ifd_scores_single_model(inputs, self.model_1)
        scores_2 = self.compute_ifd_scores_single_model(inputs, self.model_2)
    ...
    def calculate_scores(self) -> None:
        """Calculate IFD scores for all records using dual model approach and track token counts."""
        print("\n🧮 Computing dual model IFD scores and tracking tokens...")
        ...
        with tqdm(total=total_samples, desc="Processing samples", unit="samples", 
            ...
            for i in range(0, total_samples, self.batch_size):
                ...
                try:
                    scores, token_counts = self.compute_dual_ifd_scores(texts)
                    for rec, score, token_count in zip(batch, scores, token_counts):
                        rec["ifd_score"] = float(score)
                        rec["token_count"] = int(token_count)  # Store token count from IFD processing
                except Exception as e:
                    print(f"\n⚠️  Warning: Failed to process batch {i//self.batch_size + 1}: {e}")
    ...
    def create_final_dataset(self) -> List[Dict[str, Any]]:
        ...
        # Sort records by IFD score (descending - higher scores are better)
        records_sorted = sorted(self.records, key=lambda x: x["ifd_score"], reverse=True)
        ...
        print(f"📊 Dataset composition strategy:")
    ...
    def compute_final_token_stats(self, final_samples_with_metadata: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        This avoids re-tokenizing by using token counts from the IFD calculation phase.
        ...
        """
        print("🔢 Computing token statistics from IFD processing data...")
        ...
        for record in final_samples_with_metadata:
            # Use pre-computed token count from IFD processing
        ...
        print(f"✅ Token statistics computed (no re-tokenization needed):")
    ...
    def save_final_output(self, final_dataset_with_metadata: List[Dict[str, Any]]) -> None:
        ...
        # Compute token statistics using pre-computed token counts from IFD processing
        ...
        for record in final_dataset_with_metadata:
            clean_record = {k: v for k, v in record.items() if k not in ["ifd_score", "token_count"]}
        ...
        metadata = {
            ...
            "curation_method": "dual_model_ifd_superfilter",
        ...
        try:
            ...
            print(f"✅ Final dataset and metadata saved successfully!")
            ...
            print(f"   🎯 Quality: Top 75% + duplicated top 25%")
            print(f"   🔀 Shuffled: Yes (for training diversity)")
